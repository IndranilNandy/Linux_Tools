...
script_path="$LINUX_TOOLS_spring_helper"/Db_related/mysql_container_creator
script="$script_path"/docker-compose.yaml
target_compose_path="$(pwd)"/docker/mysql
env_file="$(pwd)"/.env
config_root="$curDir"/db/mysql/container
...
sql_template_ext=".mysql.template"
sel_template=
...
find_db_container() {
    ...
    mysql_container=$(docker ps --filter ancestor="$img_name" | grep "$svc_value" | awk '{print $NF}')
...
configContainer() {
    ...
    script="$sql_template_path"/default.mysql.template # default template
    ...
    mysql_container=$(find_db_container) || return 1
    user=root
    db=mysql
    psd=

    echo -n "Enter mysql '$user' password for db '$db': "
    read -s psd

    echo -e
    echo -e "db container: $mysql_container -->running--> docker exec -i $mysql_container bash -c \"sleep 20 && mysql -u $user --password=**** $db < /scriptstore/init.sql\""

    docker exec -i "$mysql_container" bash -c "echo \"Waiting for 20 secs for db to come live\" && sleep 20 && mysql -u $user --password=$psd $db < /scriptstore/init.sql"
...
copyContainerScript() {
    # echo -e "[DB][Mysql] Copying compose template and substituting env variables"
    # echo -e "[DB][Mysql] Target path: $target_compose_path"
...
init() {
    ...
    echo -e "${BLUE}${BOLD}[SPRINGSTARTER DB MYSQL CONTAINER INIT] Started.${RESET}"
    echo -e "${BLUE}${BOLD}______________________________________________________________________________________${RESET}"
    ...
    [[ -f "$target_compose_path"/docker-compose.yaml ]] &&
        echo -e "${YELLOW}[DB][Mysql] docker-compose.yaml already exists.\
    ...
    echo -e "${BLUE}${BOLD}______________________________________________________________________________________${RESET}"
    echo -e "${BLUE}${BOLD}[SPRINGSTARTER DB MYSQL CONTAINER INIT] Finished.${RESET}"
    echo -e "${BLUE}${BOLD}______________________________________________________________________________________${RESET}"
...
help() {
    cat "$help_dir"/mysql_container_configurer.help
}

prompt() {
    subcommands="$curDir"/db/mysql/container/.commands
