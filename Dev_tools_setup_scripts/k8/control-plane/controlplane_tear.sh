...
! sshpass -p "$passwd" ssh -o 'StrictHostKeyChecking no' -t "$user"@"$node" "cd ~/k8; bash --login ./k8_cluster_cleanup.sh" && echo -e "[TEAR CONTROLPLANE] FAILED!! Tearing down faced issues in control plane $node" && exit 1
