...
! sshpass -p "$passwd" ssh -o 'StrictHostKeyChecking no' -t "$user"@"$client_node" "cd ~/k8; echo $passwd | sudo -S whoami; bash --login ./nodes/clients/client_configurer.sh" && echo -e "[CLIENT CONFIGURATION] FAILED!! Configuration NOT completed in clien node $client_node" && exit 1
