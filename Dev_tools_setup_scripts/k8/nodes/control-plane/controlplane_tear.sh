...
! sshpass -p "$passwd" ssh -o 'StrictHostKeyChecking no' -t "$user"@"$node" "cd ~/k8; export psdsource=$psdsource; export k8_tear_allnodes=$k8_tear_allnodes; echo $passwd | sudo -S echo "entered"; bash --login ./cluster/k8_cluster_cleanup.sh" && echo -e "[TEAR CONTROLPLANE] FAILED!! Tearing down faced issues in control plane $node" && exit 1
