...
! sshpass -p "$passwd" ssh -o 'StrictHostKeyChecking no' -t "$user"@"$wnode" "cd ~/k8; bash --login ./cluster/k8_cluster_init.sh" --node=worker && echo -e "[WORKER] FAILED!! Setup NOT completed in worker node $wnode" && exit 1
