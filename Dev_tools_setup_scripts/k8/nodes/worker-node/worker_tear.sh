...
! sshpass -p "$passwd" ssh -o 'StrictHostKeyChecking no' -t "$user"@"$node" "cd ~/k8; echo $passwd | sudo -S whoami; bash --login ./cluster/k8_cluster_cleanup.sh" && echo -e "[TEAR WORKER] FAILED!! Tearing down faced issues in worker node $node" && exit 1
