...
! sshpass -p "$passwd" scp -r ~/MyTools/Linux_Tools/Dev_tools_setup_scripts/k8 indranilnandy@"$wnode":~ && echo -e "[WORKER] Failed to copy codebase" && exit 1
! sshpass -p "$passwd" ssh -t indranilnandy@"$wnode" "cd ~/k8; bash --login ./k8_cluster_init.sh" --node=worker && echo -e "[WORKER] FAILED!! Setup NOT completed in worker nodes" && exit 1
