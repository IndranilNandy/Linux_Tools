...
update_etc_exports() {
    grep -E -v " *#" | xargs -I X echo "grep -F \"X\" /etc/exports > /dev/null || echo \"X\" | sudo tee -a /etc/exports" | bash
}

create_non_existing_shares() {
    grep -E -v " *#" | xargs -I X echo X | awk '{ print $1 }' | xargs -I X echo "[[ -d X ]] || sudo mkdir -p X" | bash
}
...
none_owner() {
    grep -E -v " *#" | xargs -I X echo X | awk '{ print $1 }' | xargs -I X echo "sudo chown nobody:nogroup X" | bash
}

# Needed for PV recycler pod in K8 to have access to clean PV
everyone_can_modify() {
    grep -E -v " *#" | xargs -I X echo X | awk '{ print $1 }' | xargs -I X echo "sudo chmod 777 X" | bash
}
