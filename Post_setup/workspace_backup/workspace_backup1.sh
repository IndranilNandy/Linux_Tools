...
. "$curDir"/../../.systemConfig
export "$curDir"/../../.systemConfig
export $user_devroot
export $repo_root
export $workspace_backup_local
export $workspace_backup_remote
...
process() {
    ...
    (
        tmppath="$user_devroot"/"$repo_root"/"$local_repo"
...
#     if [[ -n "$ref" ]]; then
#         process $local_repo $ref
#     else
#         process $local_repo "local"
#         process $local_repo "remote"
#     fi
...
cat $wsConfig | xargs -I X echo "processWS X" | bash
